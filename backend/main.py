...
async def SingleEmotionAnalysis(document: Document):
    try:
        ...
        # 调用加载好的模型进行属性级情感分析
        singleAnalysisResult = predict(input_text, ext_model, cls_model, tokenizer, ext_id2label, cls_id2label,
                                       max_seq_len=512)
...
async def BatchEmotionAnalysis(file: UploadFile):
    ...
    try:
        ...
        filePath = "./resources/" + str(now_time) + "_" + fileName
        # 将用户上传的文件保存到本地
        ...
        if 'text' in df.columns:
            ...
            contents = df[df.columns[0]]
        # 批量文本情感分析
        batchAnalysisResults = batchPredict(contents, ext_model, cls_model, tokenizer, ext_id2label, cls_id2label,
                                            max_seq_len=512)
        # 接口结果返回
